...
//
// This is a thin delegating extension: all rewrite logic lives in `zap rewrite`,
// which is the single source of truth (src/discover/registry.rs).
// To add or change rewrite rules, edit the Rust registry — not this file.
//
// Exit code contract for `zap rewrite`:
...
// Calls `zap rewrite`; returns the rewritten command or null (pass through).
...
export default async function (pi: ExtensionAPI) {
  ...
  // Warn and bail if rtk predates 0.23.0 (when `zap rewrite` was introduced).
